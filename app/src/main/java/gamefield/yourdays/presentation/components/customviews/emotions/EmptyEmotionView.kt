...
class EmptyEmotionView @JvmOverloads constructor(
    ...
    private val viewModel =
        ViewModelProvider(context as ViewModelStoreOwner).get(EmptyEmotionViewViewModel::class.java)

    private val minusEmotionView: MinusEmotionView = MinusEmotionView(
        context = context,
        forceLightenTheme = forceLightenTheme
    ).apply { isDrawStroke = true }
    private val plusEmotionView: PlusEmotionView = PlusEmotionView(
        context = context,
        forceLightenTheme = forceLightenTheme
    ).apply { isDrawStroke = true }
    private val zeroEmotionView: ZeroEmotionView = ZeroEmotionView(
        context = context,
        forceLightenTheme = forceLightenTheme
    ).apply { isDrawStroke = true }
